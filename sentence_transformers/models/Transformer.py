...
class Transformer(nn.Module):
    ...
    def __init__(self, model_name_or_path: str, max_seq_length: int = 128,
                 ...
                 tokenizer_args: Dict = {}, isKor=False):
        ...
        if isKor:
            ...
            config = BertConfig(self.auto_model.config.vocab_size, hidden_size=768, num_hidden_layer=12,
                    num_attention_heads=12)
            self.auto_model.model = BertModel(config)
            self.tokenizer = BertTokenizer.from_pretrained(
                    './ETRI_KoBERT/003_bert_eojeol_pytorch/vocab.txt', do_lower_case=False)
        else:
    ...
    def load(input_path: str):
        ...
        return Transformer(model_name_or_path=input_path, **config, isKor=True)
